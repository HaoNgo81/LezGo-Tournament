...
describeE2E("STEP 13 remote read-only Supabase access", () => {
  ...
  it("reads standard, pool and Team vs Team snapshots through the Device B read-only boundary", async () => {
    ...
    const standardRead = await read(standardAccess.tournamentCode.toLocaleLowerCase("en"), standardAccess.shareToken);
...
async function read(tournamentCode: string, shareToken: string): Promise<{ kind: string; state: LiveTournamentState | TeamVsTeamTournamentState }> {
  const response = await readRaw(tournamentCode, shareToken);
...
async function readRaw(tournamentCode: string, shareToken: string): Promise<Response> {
  return await readAccess(new Request("http://localhost/api/supabase/tournament-access/read", {
    method: "POST",
    headers: { "x-forwarded-for": "step-13-e2e" },
...
function createStandardState(): LiveTournamentState {
  const state = createTournamentFromSetup({
    name: "STEP_13_TEST Americano",
    format: "Americano",
    playerText: Array.from({ length: 16 }, (_, index) => `STEP_13_TEST Spiller ${index + 1}`).join("\n"),
    femalePlayerText: "",
    malePlayerText: "",
    courts: 4,
    rounds: 2,
...
async function cleanupCreatedRows(createdIds: string[]): Promise<void> {
  ...
  const remainingTestRows = await restSelect<{ id: string }>("tournaments", "name=like.STEP_13_TEST*&select=id");
  const remainingAccessRows = createdIds.length
    ? await restSelect<{ id: string }>("tournament_access", `tournament_id=in.(${createdIds.join(",")})&select=id`)
    : [];

  expect(remainingTestRows).toHaveLength(0);
  expect(remainingAccessRows).toHaveLength(0);
}
