-- STEP 25I-C1-C8B-T2-FIX3 - Narrow server-only admin tournament takeover RPC.
-- The function only updates controller metadata. It does not reassign creators,
-- alter owners, delete data, or change tournament scoring/runtime state.

create or replace function public.lezgo_admin_takeover_tournament_v1(
...
begin
  ...
  set controller_user_id = p_admin_user_id,
...
revoke all on function public.lezgo_admin_takeover_tournament_v1(uuid, uuid) from public;
revoke all on function public.lezgo_admin_takeover_tournament_v1(uuid, uuid) from anon;
revoke all on function public.lezgo_admin_takeover_tournament_v1(uuid, uuid) from authenticated;
grant execute on function public.lezgo_admin_takeover_tournament_v1(uuid, uuid) to service_role;
