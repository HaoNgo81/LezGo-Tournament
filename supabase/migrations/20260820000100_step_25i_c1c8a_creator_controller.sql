-- STEP 25I-C1-C8A - Creator/controller foundation for owned tournaments.
-- Additive migration. Existing tournament rows are not deleted, reset or reassigned.

alter table public.tournaments
  add column if not exists created_by_user_id uuid null references auth.users(id) on delete set null,
  add column if not exists controller_user_id uuid null references auth.users(id) on delete set null;

update public.tournaments
set created_by_user_id = coalesce(created_by_user_id, owner_user_id),
    controller_user_id = coalesce(controller_user_id, owner_user_id)
where owner_user_id is not null
  and (created_by_user_id is null or controller_user_id is null);

create index if not exists tournaments_created_by_user_id_idx on public.tournaments(created_by_user_id);
create index if not exists tournaments_controller_user_id_idx on public.tournaments(controller_user_id);
create index if not exists tournaments_controller_status_idx on public.tournaments(controller_user_id, status);
...
using (
  ...
  or (controller_user_id is null and owner_user_id = auth.uid())
  or public.lezgo_is_admin(auth.uid())
);
...
declare
  ...
  existing_created_by_user_id uuid;
  existing_controller_user_id uuid;
  effective_controller_user_id uuid;
begin
  ...
  select owner_user_id, created_by_user_id, controller_user_id
    into existing_owner_user_id, existing_created_by_user_id, existing_controller_user_id
  from public.tournaments
  where id = saved_tournament_id
  for update;

  effective_controller_user_id := coalesce(existing_controller_user_id, existing_owner_user_id);

  if effective_controller_user_id is not null
    and effective_controller_user_id <> p_actor_user_id
    and not public.lezgo_is_admin(p_actor_user_id) then
    raise exception 'Tournament controller authorization was denied.';
  end if;
  ...
  set owner_user_id = coalesce(existing_owner_user_id, p_actor_user_id),
      created_by_user_id = coalesce(existing_created_by_user_id, existing_owner_user_id, p_actor_user_id),
      controller_user_id = coalesce(existing_controller_user_id, existing_owner_user_id, p_actor_user_id),
...
begin
  ...
  effective_controller_user_id := coalesce(tournament_row.controller_user_id, tournament_row.owner_user_id);

  if effective_controller_user_id is null
    or (effective_controller_user_id <> p_actor_user_id and not public.lezgo_is_admin(p_actor_user_id)) then
    raise exception 'Tournament controller authorization was denied.';
  end if;
