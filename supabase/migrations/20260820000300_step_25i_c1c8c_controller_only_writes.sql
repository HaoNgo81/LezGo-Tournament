-- STEP 25I-C1-C8C - Enforce controller-only management writes.
-- Replaces write RPC authorization only. No table data is deleted or rewritten.
...
begin
  ...
  if effective_controller_user_id is not null
    and effective_controller_user_id <> p_actor_user_id then
...
begin
  ...
  if effective_controller_user_id is null
    or effective_controller_user_id <> p_actor_user_id then
    raise exception 'Tournament controller authorization was denied.';
  end if;
